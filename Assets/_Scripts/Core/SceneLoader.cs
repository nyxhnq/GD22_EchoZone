...
public class SceneLoader : MonoBehaviour
{
    ...
    /// <summary>
    /// Удобный вызов: если текущая сцена — `GameScene`, запускает переход в `GameScene2` через `Loading`.
    /// Если текущая сцена другая — предупреждает и всё равно инициирует переход.
    /// </summary>
    public void SwitchGameSceneToGameScene2()
    {
        if (SceneManager.GetActiveScene().name != SceneNames.GameScene)
        {
            Debug.LogWarning("SceneLoader: текущая сцена не 'GameScene'. Инициализирую переход в 'GameScene2' через Loading.");
        }
